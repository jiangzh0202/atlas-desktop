...
class QuotationAgent:
            ...
            try:
                if already_matched and list_price > 0:
                    matched = {
                        ...
                        "pricing_mode": item.get("pricing_mode", "STANDARD"),
                        "fixed_price": item.get("fixed_price", 0),
                        "cost_with_tax": item.get("cost_with_tax", 0),
                    }
                    is_matched = True
                else:
                    part = None
                    ...
                    if part:
                        matched = {
                            ...
                            "pricing_mode": getattr(part, "pricing_mode", "STANDARD"),
                            "fixed_price": getattr(part, "fixed_price", 0),
                            "cost_with_tax": getattr(part, "cost_with_tax", 0),
                            "matched": True
                        }
                        is_matched = True
                    else:
    ...
    def compare_with_original(quote_result: dict, original_excel_path: str) -> dict:
        ...
        price_within_3pct = 0
        price_within_5pct = 0
        delta_bins = {"1-2": 0, "2-5": 0, "5-10": 0, "10+": 0}
        
        total_compared = matched + len(missing_in_ours) + len(missing_in_original)
        if total_compared > 0:
            oe_coverage_pct = round(matched / total_compared * 100, 2)
        
        for d in diff_lines:
            a = abs(d["delta_pct"])
            if a <= 2: delta_bins["1-2"] += 1
            elif a <= 5: delta_bins["2-5"] += 1
            elif a <= 10: delta_bins["5-10"] += 1
            else: delta_bins["10+"] += 1
            if a <= 1: price_within_1pct += 1
            if a <= 3: price_within_3pct += 1
            if a <= 5: price_within_5pct += 1
        
        # 没有差异的行 = 价格完全一致（delta ≤ 1%）
        price_ok = matched - len(diff_lines)
        price_within_1pct += price_ok
        price_within_3pct += price_ok
        price_within_5pct += price_ok
        ...
        return {
            ...
            "oe_coverage_pct": oe_coverage_pct,
            "price_accuracy_1pct": round(price_within_1pct / max(total_compared, 1) * 100, 2),
            "price_accuracy_3pct": round(price_within_3pct / max(total_compared, 1) * 100, 2),
            "price_accuracy_5pct": round(price_within_5pct / max(total_compared, 1) * 100, 2),
            "delta_distribution": delta_bins,
        }
